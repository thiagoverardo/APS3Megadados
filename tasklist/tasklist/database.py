...
class DBSession:
    ...
    def read_tasks(self, completed: bool = None):
        query = 'SELECT BIN_TO_UUID(uuid), description, completed, BIN_TO_UUID(uuid_user) FROM tasks'
        ...
        return {
            uuid_: Task(
                ...
                uuid_user=field_uuid_user
            )
            for uuid_, field_description, field_completed, field_uuid_user in db_results
        }
    ...
    def create_task(self, item: Task):
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                ...
                (str(uuid_), item.description, item.completed, str(item.uuid_user)),
            )
    ...
    def read_task(self, uuid_: uuid.UUID):
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                '''
                SELECT description, completed, BIN_TO_UUID(uuid_user)
                FROM tasks
        ...
        return Task(description=result[0], completed=bool(result[1]), uuid_user=result[2])
    ...
    def replace_task(self, uuid_, item):
        ...
        with self.connection.cursor() as cursor:
            cursor.execute(
                '''
                UPDATE tasks SET description=%s, completed=%s, uuid_user=UUID_TO_BIN(%s)
                WHERE uuid=UUID_TO_BIN(%s)
                ''',
                (item.description, item.completed,
                 str(item.uuid_user), str(uuid_)),
            )
        self.connection.commit()
